...
(function(global) {
    ...
    if ('UTF8' === utfType) {
      ...
      for (i = 0; i < str.length; i += 1) {
        ...
        for (j = 0; j < codePntArr.length; j += 1) {
          ...
          packed[intOffset] |=
            codePntArr[j] << (8 * (shiftModifier + bigEndianMod * (byteOffset % 4)));
          byteCnt += 1;
        }
      }
      ...
      transposeBytes =
        ('UTF16LE' === utfType && bigEndianMod !== 1) ||
        ('UTF16LE' !== utfType && bigEndianMod === 1);
      for (i = 0; i < str.length; i += 1) {
        codePnt = str.charCodeAt(i);
  ...
  function getNewState(variant) {
    ...
    if ('SHA-1' === variant && (1 & SUPPORTED_ALGS) !== 0) {
      ...
      (variant.lastIndexOf('SHA3-', 0) === 0 || variant.lastIndexOf('SHAKE', 0) === 0) &&
      (8 & SUPPORTED_ALGS) !== 0
    ) {
      for (i = 0; i < 5; i += 1) {
        retVal[i] = [
  ...
  var jsSHA = function(variant, inputFormat, options) {
    ...
    if ('SHA-1' === shaVariant && (1 & SUPPORTED_ALGS) !== 0) {
      ...
      (shaVariant.lastIndexOf('SHA3-', 0) === 0 || shaVariant.lastIndexOf('SHAKE', 0) === 0) &&
      (8 & SUPPORTED_ALGS) !== 0
    ) {
      var delimiter = 0x06;
    ...
    this.getHash = function(format, options) {
      ...
      finalizedState = finalizeFunc(
        remainder.slice(),
        remainderLen,
        processedLen,
        stateCloneFunc(intermediateState),
        outputBinLen
      );
